...
- **dlt** ingests the source into **DuckDB**, our embedded analytical (OLAP) engine, as bronze. (DuckDB is the analytical store/compute, not a lakehouse; there is no open-table-format/object-store layer.)
...
Row and column policies are enforced by translating a policy declaration into a trusted, server-side Cube filter. The instructive detail: the Cube member a policy must filter often cannot be derived from the policy's target. In AdventureWorks the country filter projects through a different physical source than the one the policy names. Rather than infer it (which would rebuild the implicit coupling we were trying to remove), the policy **declares** the member it targets. Governance mappings that cannot be derived are stated explicitly and visibly in the policy. That is what an auditor wants: to read what filter applies and why, not to reverse-engineer it.

### 5. The dataset-pack abstraction: the whole engine, made generic

A **pack** is a self-contained dataset: its source, transform, semantics, governance, and golden set, plus a manifest declaring which capabilities it has. The pipeline reads the active pack from a single seam (`GROUNDED_PACK`), and every layer, including ingestion, transform, lineage, the resolver, governance, the ungoverned control arm, and the planner, resolves against that pack. A pack can be a Postgres warehouse with a SQLMesh transform (AdventureWorks, TPC-H) or a single SQLite schema queried directly with no transform (Spider, BIRD); the pipeline branches on declared capability rather than assuming one shape.

Physically, each pack is its own DuckDB database (its own file), and a read-only `ATTACH` presents them together as one analytical store. Datasets are isolated (a bad run cannot corrupt another pack) yet queryable side by side as `pack.layer.table`. That physical identity deliberately mirrors the lineage identity in Marquez (`pack.layer:table`): the data store and the lineage graph address things the same way.
