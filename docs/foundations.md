...
- **The normalizer is the load-bearing glue.** It must map dlt's `load_info` and SQLMesh's column lineage into valid OpenLineage faithfully and **without hardcoding**. It is derived from the tools' own metadata so it works on any dataset (see slice 033b). This is where correctness lives and where the engineering shows.
