...
![Type: research thesis](https://img.shields.io/badge/type-research%20thesis-8A2BE2)
![Status: reference build (not a product)](https://img.shields.io/badge/status-reference%20build%20%E2%80%94%20not%20a%20product-lightgrey)

> ### 📌 This is a research thesis and reference build, not a product.
> Grounded is an **independent research project** built to test and demonstrate a single idea, with a published evaluation as the evidence. It is **not** production software and **not** something to adopt: there is no roadmap, no releases, no support, no SLAs, and no intent to deploy it as-is. Read it as *an argument backed by measurement* — "here is a design for governed AI analytics, here is what it does and does not achieve, and here is the honest data."

**The thesis, in one line:** moving free-form SQL out of the model's hands — the model routes one *declared* metric call instead of writing SQL — can make an AI's analytical answers more trustworthy, inspectable, and testable; and that claim should be *measured*, not assumed.

As a reference build, Grounded makes that idea concrete: instead of asking a model to write unrestricted SQL, it validates one declared MCP tool call, executes a governed metric, applies policy, verifies the result, records an audit event, and returns a lineage citation. Because the agent can invoke only declared metric calls, it cannot author free-form SQL at the boundary. Selecting the wrong declared call remains possible, so routing and coverage still depend on model quality and are measured, not assumed.
...
Because the model routes a declared call instead of authoring SQL, it cannot fabricate a free-form query at the boundary; it can still select the wrong declared call, so that error is measured rather than assumed. The primary metric is correct answers over **all** in-catalog attempts, one denominator for every model and both arms. For the capable models (gemma2:9b, phi4, qwen2.5:14b, qwen2.5:7b), governed correct/all ranged from 78.9% to 93.0% on AdventureWorks, 75.0% to 98.7% on TPC-H, 96.8% to 100% on Spider world_1, and 87.0% to 100% on BIRD california_schools. Measured only over the cases a model actually answered (correct-when-answered), the same models ranged 78.9–94.3%, 93.4–98.7%, 96.8–100%, and 95.2–100% respectively; the gap between the two is coverage — a model can decline or fail to answer part of the in-catalog set rather than answer it wrongly (for example gemma2:9b answers only 76% of TPC-H). Governed wrong-answer rates ran from the low single digits up to about 21% on AdventureWorks. The raw-SQL control arm was far less accurate, but not uniformly near-zero: when the same two-decimal rounding is applied to both arms, raw-SQL correct/all reaches 14.1% to 22.5% for the four capable models on AdventureWorks (versus 78.9–93.0% governed), stays at 0% on TPC-H, and runs in the low single digits up to about 13% on Spider world_1 and BIRD california_schools. A sampled, heuristic inspection of TPC-H raw failures found genuine wrong answers — incorrect aggregations and joins, and references to columns the gold schema dimensionalized away; my own audit (script and summary published in the repo) since replayed the eligible stored raw-SQL attempts and recovered the governed results affected by legacy truncation, and found no correct-to-wrong or wrong-to-correct changes among the compared results (replay errors reported separately), while a fully adjudicated root-cause classification of each raw failure remains heuristic (see [benchmarks.md](docs/benchmarks.md)). For the capable models, governed correctness is substantially higher than the raw-SQL arm on every pack. The boundary does not rescue a weak model, though: on AdventureWorks, qwen2.5:3b produced correct answers on 0/213 governed attempts versus 24/213 raw (11.3%). Run variance was effectively zero at temperature zero. Full methodology, tables with denominators, and the disclosed comparison caveats are in [benchmarks.md](docs/benchmarks.md).
